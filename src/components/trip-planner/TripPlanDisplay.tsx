...
import { MapIcon, RouteIcon, ClockIcon, InfoIcon, SparklesIcon, NavigationIcon, CloudyIcon, SunIcon, ZapIcon, SnowflakeIcon, UmbrellaIcon, WindIcon, ThermometerIcon } from 'lucide-react';
...
function getWaypointWeatherIcon(condition: string): JSX.Element {
  ...
  if (lowerCondition.includes('fog')) return <CloudyIcon className="h-5 w-5 text-gray-500" />; // Using Cloudy for fog too
  return <ThermometerIcon className="h-5 w-5 text-gray-400" />; // Default for temperature or other conditions
}
  ...
  return (
    <Card className="mb-6 shadow-md border-border bg-card">
      ...
      <CardContent>
        ...
        {route.mapImageUrl && (
          <div className="my-4 rounded-lg overflow-hidden border border-border shadow-sm aspect-[16/9] relative">
            <Image
              ...
              data-ai-hint={route.mapImageHint}
            />
          </div>
        )}
